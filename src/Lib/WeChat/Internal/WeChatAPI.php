...
namespace App\Lib\WeChat\Internal;
...
{
	const INTERNAL_WECHAT_TOKEN_PREFIX = 'wechat:internal:token:';

	const AHEADTIME = 1000;

	const INTERNAL_JSSDK_SHORT = 'jssdk:internal:short:%s';

	const INTERNAL_JSSDK_LONG = 'jssdk:internal:long:%s';
	...
	public function __construct($container) {
		...
		$this->config = $container->getParameter('wechat.internal');
	}
	...
	public function getWeChatUserInfo() 
	{
		...
		if ($code) {
			$access_token = $this->getSnsAccessToken($code);
			if(isset($access_token->openid)) {
				...
				if($access_token->scope == 'snsapi_userinfo') {
					return $this->getSnsUserInfo($access_token->openid, $access_token->access_token);
				}
			}
		}
		return null;
	...
	{
		$request = $this->container->get('http_kernel')->getRequest();
		$url = $request->generateUrl('wechat/oauth/callback', [], true);
		return $this->getAuthorizeUrl($url);
	}
	...
	public function retrieveAccessToken() {
		...
		$url = sprintf($applink, $this->config['appid'], $this->config['appsecret']);
	...
	public function getAuthorizeUrl($callback) {
		...
		$url = sprintf($applink, $this->config['appid'], $redirect_uri, $this->config['scope']);
		return $url;
	}
	...
	public function getSnsAccessToken($code) {
		...
		$url = sprintf($applink, $code, $this->config['appid'], $this->config['appsecret']);
		$return = file_get_contents($url);
		return json_decode($return);
	}
	...
	private function getAccessKey($type = 'access_token') {
		$key = self::INTERNAL_WECHAT_TOKEN_PREFIX . $type;
  		if($key_value = $this->container->get('redis')->get($key)) {
    		return $key_value;
  		} else {
    		if($type == 'access_token') {
  				...
  				if(isset($data->access_token)) {
	        		...
	        		$expires_in = $data->expires_in - self::AHEADTIME;
  				}
		    } else {
		    ...
		    $this->container->get('redis')->set($key, $key_value);
			$this->container->get('redis')->setTimeout($key, $expires_in);
			return $key_value;
  		}
	}
	...
	public function jssdkShortConfig($debug = false)
	{
		$request = $this->container->get('http_kernel')->getRequest();
		$referer = $request->query->get('referer') ? $request->query->get('referer') : $request->getHttpReferer();
		$key = sprintf(self::INTERNAL_JSSDK_SHORT, md5($referer . $debug));
		if($key_value = $this->container->get('redis')->get($key)) {
    		return $key_value;
  		}
		$jsApi = $this->getJSApiTicketFromWechat();
		if($jsApi){
			$config = $this->jssdkConfig($referer, $jsApi->ticket, $debug);
			$key_value = "wx.config(".json_encode($config, JSON_UNESCAPED_UNICODE).");\n";
			$expires_in = $jsApi->expires_in - self::AHEADTIME;
		    $this->container->get('redis')->set($key, $key_value);
			$this->container->get('redis')->setTimeout($key, $expires_in);
			return $key_value;
		}
	...
	public function jssdkLongConfig($debug = false)
	{
    	$request = $this->container->get('http_kernel')->getRequest();
		$referer = $request->query->get('referer') ? $request->query->get('referer') : $request->getHttpReferer();
		$key = sprintf(self::INTERNAL_JSSDK_LONG, md5($referer . $debug));
		if($key_value = $this->container->get('redis')->get($key)) {
    		return $key_value;
  		}
		$jsApi = $this->getJSApiTicketFromWechat();
